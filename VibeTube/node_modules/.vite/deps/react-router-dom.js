import { i as __toESM, n as __commonJS, t as require_react } from "./react-CdzKsccS.js";
import { t as require_react_dom } from "./react-dom-B_D87pwJ.js";

//#region node_modules/.deno/react-router@7.13.2/node_modules/react-router/dist/development/chunk-UVKPFVEO.mjs
...
function createRouter(init) {
	...
	async function handleAction(request, location$1, submission, matches, scopedContext, isFogOfWar, initialHydration, opts = {}) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) result = {
			...
			let results = await callDataStrategy(request, location$1, getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location$1, matches, actionMatch, initialHydration ? [] : hydrationRouteProperties2, scopedContext), scopedContext, null);
	...
	async function handleLoaders(request, location$1, matches, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult, callSiteDefaultShouldRevalidate) {
		...
		let { loaderResults, fetcherResults } = await callLoadersAndMaybeResolveData(dsMatches, revalidatingFetchers, request, location$1, scopedContext);
		if (request.signal.aborted) return { shortCircuited: true };
	...
	async function callLoadersAndMaybeResolveData(matches, fetchersToLoad, request, location$1, scopedContext) {
		let loaderResultsPromise = callDataStrategy(request, location$1, matches, scopedContext, null);
...
function createStaticHandler(routes, opts) {
	...
	async function query(request, { requestContext, filterMatchesToLoad, skipLoaderErrorBubbling, skipRevalidation, dataStrategy, generateMiddlewareResponse, unstable_normalizePath } = {}) {
		...
		let location$1 = createLocation("", normalizePath(request), null, "default");
		...
		if (generateMiddlewareResponse) {
			...
			try {
				...
				let response = await runServerMiddlewarePipeline({
					request,
					unstable_url: createDataFunctionUrl(request, location$1),
	...
	async function queryImpl(request, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, skipRevalidation) {
		...
		try {
			if (isMutationMethod(request.method)) return await submit(request, location$1, matches, routeMatch || getTargetMatch(matches, location$1), requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch != null, filterMatchesToLoad, skipRevalidation);
			let result = await loadRouteData(request, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad);
	...
	async function submit(request, location$1, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest, filterMatchesToLoad, skipRevalidation) {
		let result;
		if (!actionMatch.route.action && !actionMatch.route.lazy) {
			...
			result = (await callDataStrategy(request, location$1, getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location$1, matches, actionMatch, [], requestContext), isRouteRequest, requestContext, dataStrategy))[actionMatch.route.id];
		...
		if (isErrorResult(result)) return {
			...await loadRouteData(loaderRequest, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, filterMatchesToLoad, [(skipLoaderErrorBubbling ? actionMatch : findNearestBoundary(matches, actionMatch.route.id)).route.id, result]),
			statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
			actionData: null,
			actionHeaders: { ...result.headers ? { [actionMatch.route.id]: result.headers } : {} }
		};
		return {
			...await loadRouteData(loaderRequest, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, filterMatchesToLoad),
			actionData: { [actionMatch.route.id]: result.data },
			...result.statusCode ? { statusCode: result.statusCode } : {},
			actionHeaders: result.headers ? { [actionMatch.route.id]: result.headers } : {}
		};
	}
	async function loadRouteData(request, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, pendingActionResult) {
		...
		let dsMatches;
		if (routeMatch) dsMatches = getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location$1, matches, routeMatch, [], requestContext);
		else {
			...
			dsMatches = matches.map((match, index) => {
				if (maxIdx != null && index > maxIdx) return getDataStrategyMatch(mapRouteProperties2, manifest, request, location$1, pattern, match, [], requestContext, false);
				return getDataStrategyMatch(mapRouteProperties2, manifest, request, location$1, pattern, match, [], requestContext, (match.route.loader || match.route.lazy) != null && (!filterMatchesToLoad || filterMatchesToLoad(match)));
			});
		}
		...
		let results = await callDataStrategy(request, location$1, dsMatches, isRouteRequest, requestContext, dataStrategy);
		if (request.signal.aborted) throwStaticHandlerAbortedError(request, isRouteRequest);
		return {
	...
	async function callDataStrategy(request, location$1, matches, isRouteRequest, requestContext, dataStrategy) {
		let results = await callDataStrategyImpl(dataStrategy || defaultDataStrategy, request, location$1, matches, null, requestContext, true);
		let dataResults = {};
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, history, state, matches, submission, location$1, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, hasPatchRoutesOnNavigation, pendingActionResult, callSiteDefaultShouldRevalidate) {
	...
	let dsMatches = matches.map((match, index) => {
		...
		if (forceShouldLoad !== null) return getDataStrategyMatch(mapRouteProperties2, manifest, request, location$1, pattern, match, lazyRoutePropertiesToSkip, scopedContext, forceShouldLoad);
		...
		return getDataStrategyMatch(mapRouteProperties2, manifest, request, location$1, pattern, match, lazyRoutePropertiesToSkip, scopedContext, shouldRevalidateLoader(match, shouldRevalidateArgs), shouldRevalidateArgs, callSiteDefaultShouldRevalidate);
	});
...
//#endregion
//#region node_modules/.deno/cookie@1.1.1/node_modules/cookie/dist/index.js
var require_dist = /* @__PURE__ */ __commonJS({ "node_modules/.deno/cookie@1.1.1/node_modules/cookie/dist/index.js": ((exports) => {
...
//#endregion
//#region node_modules/.deno/set-cookie-parser@2.7.2/node_modules/set-cookie-parser/lib/set-cookie.js
var require_set_cookie = /* @__PURE__ */ __commonJS({ "node_modules/.deno/set-cookie-parser@2.7.2/node_modules/set-cookie-parser/lib/set-cookie.js": ((exports, module) => {
...
//#endregion
//#region node_modules/.deno/react-router@7.13.2/node_modules/react-router/dist/development/chunk-UALY5CBT.mjs
/**
* react-router v7.13.2
*
* Copyright (c) Remix Software Inc.
...
async function callRouteHandler(handler, args, future) {
	let result = await handler({
		request: future.unstable_passThroughRequests ? args.request : stripRoutesParam(stripIndexParam$1(args.request)),
...
//#endregion
//#region node_modules/.deno/react-router@7.13.2/node_modules/react-router/dist/development/dom-export.mjs
/**
* react-router v7.13.2
*
* Copyright (c) Remix Software Inc.
...
//#endregion
//#region node_modules/.deno/react-router-dom@7.13.2/node_modules/react-router-dom/dist/index.mjs
/**
